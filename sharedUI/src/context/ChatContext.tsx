...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initFromURLAndFetch = async () => {
      ...
      if (typeof window !== "undefined" && !IS_EXTENSION) {
        ...
        urlSessionId = pathParts.length >= 3 && pathParts[1] === "chat" ? pathParts[2] : null;
      }
  ...
  const createSession = useCallback(async (): Promise<string> => {
    ...
    try {
      ...
      // Update URL langsung setelah membuat sesi baru (untuk website)
      if (typeof window !== "undefined" && !IS_EXTENSION) {
        const expectedPath = `/chat/${newSession.id}`;
  ...
  const sendMessage = useCallback(async ({ text, signal, sessionId, attachments }: SendMessageArgs) => {
    ...
    try {
      ...
      // Update URL jika mengirim pesan tanpa sesi aktif (untuk website)
      if (typeof window !== "undefined" && !IS_EXTENSION && !sessionId && activeSessionId !== targetSession) {
        const expectedPath = `/chat/${targetSession}`;
        if (window.location.pathname !== expectedPath) {
          window.history.replaceState({}, '', expectedPath);
        }
      }
